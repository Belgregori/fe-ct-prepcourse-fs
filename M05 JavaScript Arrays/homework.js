...
function incrementarPorUno(array) {
   ...
   // Tu código:
   arraySuma = array.map( (num) => { return num + 1});
   return(arraySuma);
}
...
function arrayContiene(array, elemento) {
   ...
   // Tu código:
   var hayElemento = array.includes(elemento);
   return(hayElemento);
}
...
function empiezaConNueve(num) {
   ...
   var numToString = numToString.split("");
   if( numToString[0]==9) {
      return true;
   }else{
      return false;
...
function todosIguales(array) {
   ...
   const sonIguales = array.every((num)=>array[0]===num);
   if(sonIguales){
      return true;
   } else{
      return false;
...
function continueStatement(num) {
   ...
   let i =0;
   while(i<10){
      i++; //aumentamos el iterador en uno 
      if(i===5){
         continue;
      }else{
         num +=2; //num se incrementa en dos
         newArreglo.push(num);
      }
   }
return newArreglo;
